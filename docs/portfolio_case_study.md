...
- `research_notebooks/gwrf_vs_priority_correlation.png`의 `R²=0.006`은 legacy GWRF 정규화 위험도와 09번 시설 입지 선정 정규화 점수 사이의 선형 설명력이 매우 낮음을 뜻합니다. R²는 순위상관 지표가 아니며, 모델 실패가 아니라 서로 다른 위험 개념을 측정할 가능성을 보여주는 추가 현장 검토용 진단 자료입니다.
