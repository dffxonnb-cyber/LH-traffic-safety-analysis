...
| `needs confirmation` | 생성 로직이나 언급은 있으나 최종 공개 결과 파일은 확인 불가 |
...
| Public Top-20 preview | `confirmed public artifact` | [public_top20_priority.csv](data/public_top20_priority.csv), [public-top20-priority-preview.svg](images/public-top20-priority-preview.svg) | 공개 가능한 격자 순위와 정규화 위험도는 직접 확인 가능 |
...
3. 후보 안정성 참고값: Monte Carlo mean Jaccard `0.503`.
4. 공개 후보 미리보기: [public_top20_priority.csv](data/public_top20_priority.csv)에 있는 상위 20개 격자와 정규화 위험도.
5. 검증 경계: fold별 원본, run-level Monte Carlo, 시설 패키지·추천 사유 원본은 공개 저장소에서 재검산할 수 없음.
6. 대시보드 경계: dashboard 코드는 공개되어 있으나, 공개 배포 URL은 검증 가능한 evidence로 사용하지 않음.
...
| 3 | [public_evidence_status.csv](data/public_evidence_status.csv) | 근거별 확인 가능 / limited / needs confirmation / not available 상태 확인 |
