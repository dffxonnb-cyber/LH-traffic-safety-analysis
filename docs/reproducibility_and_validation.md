...
- 공개 진단 결과: `R²=0.006`
- 해석: 두 점수 사이에서 선형 관계로 설명되는 변동이 매우 적다는 뜻이다. R²는 순위상관 지표가 아니므로 순위 일치 여부를 직접 증명하지 않으며, 모델 실패를 직접 의미하지도 않는다. 서로 다른 위험 개념과 가중치를 반영할 수 있으므로 별도 신호로 비교하고 현장에서 확인해야 한다.
