...
핵심은 단순히 위험지도를 만드는 것이 아니라, `위험도 산정 → 지역 간 전이 검증 → 후보 격자 우선순위 도출 → 시설 패키지 추천 → 적용 전/후 시나리오 시각화`를 하나의 의사결정 흐름으로 연결한 점입니다.

이 프로젝트는 공간 데이터를 다뤘지만, 본질적으로는 `제한된 예산과 자원을 어디에 먼저 배분할 것인가`를 다루는 의사결정 문제입니다.  
따라서 분석의 목표는 높은 위험도를 예측하는 데서 끝나지 않고, **왜 이 격자가 우선순위가 높은지**, **어떤 시설 개입이 필요한지**, **개입 후 위험도가 어떻게 달라질 수 있는지**까지 설명 가능한 형태로 만드는 데 있었습니다.
...
| Training Context | 사고 이력이 상대적으로 충분한 4개 시·구의 교통안전 패턴 |
| Spatial Unit | 격자 단위 위험도 산정 및 후보지 비교 |
| Core Method | 기존 도시에서 학습한 위험 패턴을 하남교산 격자에 전이 적용 |
...
| Output | 위험 격자, 우선 설치 후보, 추천 시설 패키지, 추천 사유, 전/후 시나리오 시각화 |
...
| Explanation | `recommended_package`, `recommendation_reason`으로 추천 근거를 설명한다 |
...
|----------|--------|---------|-----------|
| Transfer Validation | Mean AUC `0.8604` | 지역을 하나씩 홀드아웃해도 위험 구간을 구분하는 성능이 유지됨 | [reproducibility_and_validation.md](./docs/reproducibility_and_validation.md#top35-validation) |
| Hotspot Capture | Mean Top-10% Lift `4.39x` | 상위 위험 구간이 무작위 선정 대비 훨씬 높은 사고 집중도를 보임 | [reproducibility_and_validation.md](./docs/reproducibility_and_validation.md#top35-validation) |
| Worst Case | Worst holdout AUC `0.7979` | 가장 불리한 지역에서도 해석 가능한 수준의 전이 성능을 확보 | [reproducibility_and_validation.md](./docs/reproducibility_and_validation.md#top35-validation) |
| Selection Robustness | Monte Carlo mean Jaccard `0.503` | Top20 후보가 시나리오 변화에 완전히 무너지지 않음을 확인 | [reproducibility_and_validation.md](./docs/reproducibility_and_validation.md#top35-validation) |
| Explainability | `recommended_package`, `recommendation_reason` 컬럼 제공 | “왜 이 위치에 이 시설인가”를 슬라이드/Q&A 수준으로 설명 가능 | [reproducibility_and_validation.md](./docs/reproducibility_and_validation.md#top35-validation) |
...
- `recommended_package`로 설치 패키지를 제안
- `recommendation_reason`으로 추천 근거를 설명
...
| Explainability | 위험도 정의, GRF 전이 논리, 교산 적용 배경을 각각 별도 문서로 분리 |
| Public Release Policy | 원본 데이터 제외, 검증 가능한 시각화·문서·CSV만 공개 |
| Decision Output | `recommended_package`, `recommendation_reason` 기반으로 후보지별 추천 사유 제공 |
...
- [GRF Risk Methodology](./docs/grf_risk_methodology.md)
