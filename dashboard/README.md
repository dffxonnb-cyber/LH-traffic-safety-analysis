...
필수 GeoJSON/CSV가 연결되지 않은 상태로 앱을 열면, 대시보드가 바로 오류로 중단되지 않고 `public-safe fallback` 화면이 열립니다.

- README와 `docs/` 기준으로 검토 가능한 핵심 시각화
- 누락된 필수 데이터 경로 안내
- 공개 저장소에서 먼저 읽어야 할 문서 링크

즉, 공개 저장소만 보는 리뷰어는 결과를 먼저 확인할 수 있고, 승인 데이터를 가진 사용자는 같은 앱에서 실제 지도 탐색으로 넘어갈 수 있습니다.
